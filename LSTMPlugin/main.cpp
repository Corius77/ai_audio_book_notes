...
void processSignalState(std::string inWav, std::string outWav, torch::jit::script::Module& model){
    ...
    audioFile.load(inWav);
    std::vector<float> signal = interleaveAudio(audioFile.samples);
  
    LSTMState state = getRandomStartState(1, 3);
    int blockSize = 1024;
    ...
    std::vector<std::vector<float>> signal_buffor = deinterleaveAudio(signal, numChannels);
    bool ok = audioFile.setAudioBuffer(signal_buffor);
    audioFile.save(outWav);
}
